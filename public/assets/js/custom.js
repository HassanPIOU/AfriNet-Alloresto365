  ...
  var Custom = {
    ...
    extraAreaMorrisChart: function() {
      ...
      function generateData() {
          ...
          var dataMorris = $('#dataMorris').val()
          var dataMorrisClients = $('#dataMorrisClients').val()
          dataMorris =  JSON.parse(dataMorris)
          dataMorrisClients =  JSON.parse(dataMorrisClients)

        var ele = null;
        var data = [];
        for( var i=0; i<dataMorris.length; i++) {
          ele = {
            period: dataMorris[i].date+'',
            abonnement : dataMorris[i].nbre,
            client : dataMorrisClients[i].nbre,
          }
          data.push( ele );
        }
      ...
      var chart = Morris.Area({
        ...
        data: generateData(),
        lineColors: ["#bc1a1f","#2599bf"],
        xkey: 'period',
        ykeys: ['abonnement','client'],
        labels: ['Abonnements','Clients'],
        lineWidth: 0,
        fillOpacity: 1.0,
        pointSize: 0,
        pointFillColors: ['#fff','#fff'],
        pointStrokeColors: ["#3ec72e","#2599bf"],
